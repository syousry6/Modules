...
resource "aws_security_group" "alb" {
  ...
  name  = join("-", [local.module_prefix, "sg"])
  tags        = local.tags
...
# S3 Bucket
 module "access_logs" {
   source    = "git::https://github.com/cloudposse/terraform-aws-lb-s3-bucket.git?ref=tags/0.14.0"
   enabled   = var.create
...
resource "aws_lb_target_group" "alb" {
  ...
  name  = lower(join("-", [local.module_prefix, var.target_groups[count.index].protocol]))
  tags  = local.tags
