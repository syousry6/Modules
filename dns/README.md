...
The purpose of this module is to create Route 53 private or public hosted zone and also NS record sets.
...
data "terraform_remote_state" "vpc" {
  backend = "s3" 
  config  = {
      bucket  = "macewan-s3-backend"
      key     = "ca-central-1/vpc/terraform.tfstate"
      region  = "ca-central-1"
  }
}

module "dns" {
  source                     = "git::git@github.com:macewanu/onica-poc-modules.git//aws/dns?ref=v0.1.0"
  vpc_id                     = data.terraform_remote_state.vpc.outputs.vpc_id
  parent_domain_name         = "macewan.co"
  aws_subdomain_name         = "poc.aws"
...
output "dns_public_zone_id" {
  value = join("", module.dns.*.dns_public_zone_id)
}

output "dns_public_zone_name" {
  value = join("", module.dns.*.dns_public_zone_name)
}

output "dns_public_zone_name_servers" {
  value = flatten(module.dns.*.dns_public_zone_name_servers)
}

# Private Zone

output "dns_private_zone_id" {
  value = join("", module.dns.*.dns_private_zone_id)
}

output "dns_private_zone_name" {
  value = join("", module.dns.*.dns_private_zone_name)
}

output "dns_private_zone_name_servers" {
  value = flatten(module.dns.*.dns_private_zone_name_servers)
}
...
terraform {
  backend "s3" {
    ...
    region = "ca-central-1"
    key = "ca-central-1/dns/terraform.tfstate"
  }
}
...
variable terraform_module {
  type        = string
  default     = "macewan/onica-poc-modules/aws/dns"
